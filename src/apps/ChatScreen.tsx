        ...
        <div className="space-y-2">
          ...
          <button
            onClick={async () => {
              ...
              // 查找用户最近发送的图片
              const recentUserImages = [...thread.messages]
                .reverse()
                .filter(m => m.role === 'user' && m.media && m.media.some(md => md.kind === 'image'))
                .slice(0, 1);

              if (recentUserImages.length > 0 && recentUserImages[0].media) {
                const imageMedia = recentUserImages[0].media.find(md => md.kind === 'image');
                if (imageMedia && 'url' in imageMedia) {
                  ...
                  // 角色发送温馨反应
                  sendActive('你刚刚换上了用户发的情侣头像。请温馨浪漫地表达你的喜悦和甜蜜，说你很喜欢这个头像。10-50字。');
                } else {
                  sendActive('用户刚刚暗示想换情侣头像。请温馨浪漫地回应，并说你也准备换上配对的头像。10-50字。');
                }
              } else {
                sendActive('用户刚刚暗示想换情侣头像。请温馨浪漫地回应，并说你也准备换上配对的头像。10-50字。');
              }
